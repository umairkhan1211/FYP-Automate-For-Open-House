...
export default async function handler(req, res) {
  ...
  if (method === "PUT") {
    ...
    console.log("supervisor video review ma kya api ma ", req.body);

    try {
      // Find the supervisor by ID and update the supervisorVideoReview status
      const supervisor = await Status.findOneAndUpdate(
        { studentId, supervisorId },
        {
          supervisorVideoReview: status,

          videoStatus: status, // ✅ Yeh bhi ab update hoga
          updatedAt: new Date(),
        },
        { new: true }
      );
      ...
      return res
        .status(200)
        .json({ message: "Supervisor video review status updated", supervisor });
    } catch (error) {
      return res
        .status(500)
