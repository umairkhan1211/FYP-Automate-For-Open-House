// pages/api/SupervisorUpdateStatus.js
  ...
  if (method === "PUT") {
    ...
    try {
      ...
      const supervisor = await Status.findOneAndUpdate(
        ...
        { supervisorCvReview: status, updatedAt: new Date() },
        { new: true }
      );
      ...
      return res.status(200).json({ message: "Supervisor CV review status updated", supervisor });
    } catch (error) {
