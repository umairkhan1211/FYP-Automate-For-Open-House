...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (alreadyExists) {
      ...
      if (alreadyExists.bannerStatus !== "uploaded") {
        alreadyExists.bannerStatus = "uploaded"; // Update the fypStatus
        await alreadyExists.save(); // Save the updated status
        return res.status(200).json({ message: "bannerStatus updated successfully" });
      }
      return res.status(200).json({ message: "bannerStatus already uploaded" });
    }
    ...
    return res.status(201).json({ message: "Banner status updated successfully" });
  } catch (error) {
    console.error("BannerSubmit error:", error);
