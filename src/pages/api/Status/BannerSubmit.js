  ...
  try {
    ...
    const updatePromises = groupMembers.map(async (member) => {
      ...
      if (existingStatus) {
        ...
        if (existingStatus.bannerStatus !== "uploaded") {
          existingStatus.bannerStatus = "uploaded";
    ...
    return res.status(200).json({ message: "Banner status updated for all group members" });
  } catch (error) {
