...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (alreadyExists) {
      ...
      if (alreadyExists.cvStatus !== "uploaded") {
        alreadyExists.cvStatus = "uploaded"; // Update the fypStatus
    ...
    console.log("cvstatus dekha ",newStatus)

    await newStatus.save();

    return res.status(201).json({ message: "CV status updated successfully" });
  } catch (error) {
    console.error("CVSubmit error:", error);
