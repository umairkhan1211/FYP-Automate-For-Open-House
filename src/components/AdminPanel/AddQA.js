...
function AddQA({ darkMode }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const studentData = { ...data, role: "QA" }; // Automatically setting role as "Supervisor"
      const response = await axios.post("/api/users/signup", studentData);
      console.log(response.data);

      toast.success("QA Member Added Successfully!");
      ...
      toast.error("Failed to Add QA Member");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={`flex items-center justify-center w-full h-full overflow-hidden ${darkMode ? "dark" : ""}`}>
      <div className="w-full max-w-md  dark:bg-slate-800 border-2 border-[#0069D9] dark:border-white rounded-md shadow-2xl p-6 overflow-auto max-h-[85vh]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <input type="hidden" {...register("role")} value="QA" />
      ...
      <div className="absolute bottom-4 right-4 opacity-0 animate-fade-in">
        <iframe
          src="https://lottie.host/embed/5ed1b4ec-30f3-40da-a58d-0a9a6641ab74/u6aodClAPn.lottie"
          className="w-24 h-24"
        ></iframe>
      </div>
    </div>
