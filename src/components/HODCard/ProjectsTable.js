// components/HODCard/ProjectsTable.js
...
const ProjectsTable = ({ projects }) => {
  ...
  const displayProjects = Object.entries(groupedProjects).flatMap(([title, group]) => {
  ...
  // Pagination Logic
  const totalPages = Math.max(1, Math.ceil(displayProjects.length / entriesPerPage));
  const startIndex = (currentPage - 1) * entriesPerPage;
  const paginatedData = displayProjects.slice(startIndex, startIndex + entriesPerPage);
              ...
              {paginatedData.map((item, index) => {
                if (item.isGroupHeader) {
                  ...
                  return (
                    ...
                    >
                      {/* <td className="px-4 py-3 font-bold" rowSpan={item.groupMembers.length + 1}>
                        {startIndex + index + 1}
                      </td> */}
                      <td className="px-4 py-3 font-bold capitalize" rowSpan={item.groupMembers.length + 1}>
                        {item.projectTitle}
                      </td>
                      <td className="px-4 py-3 font-bold capitalize" rowSpan={item.groupMembers.length + 1}>
                        {item.supervisorName}
                      </td>
                      {/* <td colSpan="5" className="px-4 py-3 font-bold text-left">
                        Group Members ({item.groupMembers.length})
                      </td> */}
                    </motion.tr>
                  );
                } else {
                  // This is a member row
                  const project = item;
                  return (
                    <motion.tr
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="grid grid-cols-2 gap-1">
                          ...
                          <div>
                            <span className="text-xs block">Video: </span>
                            <StatusBadge status={project.supervisorVideoReview} />
                          </div>
                          <div>
                            <span className="text-xs block">Banner: </span>
                            <StatusBadge status={project.supervisorBannerReview} />
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-sm font-medium">{calculateCompletion(project)}%</span>
                        </div>
                      </td>
                    </motion.tr>
                  );
